...
def modifiy_created_at(df):
	for index, row in df.iterrows():
		...
		time_list = m.group(3).split(":")
		x = str(time_list.pop(0)) + "." + ("".join(time_list))
		df.set_value(index,'Created_at',x)
		df.set_value(index,'Month',str(m.group(2)))
	return df
